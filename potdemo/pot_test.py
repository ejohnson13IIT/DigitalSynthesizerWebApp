...
def map_pot_to_octave(pot_value):
    """Map pot1 (0-1) to discrete octave (-2, -1, 0, 1, 2)"""
...
def map_pot_to_semitone(pot_value):
    """Map pot2 (0-1) to semitone (-12.00 to 12.00) continuous"""
    return pot_value * 24.0 - 12.0

def map_pot_to_mix(pot_value):
    """Map pot3 (0-1) to mix (0.00 to 1.00) continuous"""
    return pot_value
...
while True:
	...
	if abs(int(pot4.value * 100) - lastVal4) > 2:
		client.send_message("/Carla/0/set_parameter_value", [3, pot4.value])
